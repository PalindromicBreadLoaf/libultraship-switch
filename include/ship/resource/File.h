...
struct File {
    ...
    /** @brief Real entry byte count. Backends over-allocate Buffer (archive loaders reserve
     *  +4096 of guard space), so Buffer->size() is not the entry size. 0 means unset by the
     *  backend; fall back to Buffer->size() then. */
    size_t TrueSize = 0;
